const APP_VERSION="V28.0.8 Production";
const RELEASE_DATE="2026-07-27";
const RELEASE_SUMMARY="접수 알림 관리자 전용 · 결제금액 기준 입금 절차 · 직원 숙박 이용 신청서 · 카카오 로그 개선";
...
async function submitReservation(e,roomId){
  ...
  const item={id:uid(),userId:user().id,userType:isCustomer?"customer":"employee",userName:user().name,dept:user().dept||"",roomId,roomName:room.name,checkin,checkout,nights,people,extraPeople:Math.max(0,people-room.basePeople),useType,discountRate:policy.discountRate,paymentRequired,amount,bankInfo:`${state.settings.bankName} ${state.settings.bankAccount} ${state.settings.bankHolder}`,phone:f.get("phone"),payer:f.get("payer"),memo:f.get("memo"),status:"대기",checkinStatus:paymentRequired?"입금대기":"예약대기",paymentDueAt,refundPolicySnapshot:refundRulesText(),vehicleUse,vehicleStatus,vehicleStart,vehicleEnd,source:isCustomer?"public":"welfare",createdAt:new Date().toLocaleString(),createdAtIso:new Date().toISOString()};
  try{
...
function reservationTable(rows,admin){
  ...
  return`<table class="table"><thead><tr>${admin?`<th>선택</th>`:""}<th>숙소</th><th>신청자</th><th>기간</th><th>구분/금액</th><th>인원</th><th>차량</th><th>상태</th><th>관리</th></tr></thead><tbody>${rows.map(r=>`<tr>${admin?`<td>${rowCheck('chkReservation',r.id)}</td>`:""}<td>${r.roomName}</td><td>${r.userName}<br><span class="muted">${r.dept||""}</span></td><td>${r.checkin} ~ ${r.checkout}<br><b>${r.nights||calcNights(r.checkin,r.checkout)}박</b></td><td>${r.useType||"-"}<br>${r.amount?`<b>${money(r.amount)}</b><br><span class="muted">${r.bankInfo||""}</span>`:"무료"}</td><td>${r.people}명${r.extraPeople?`<br><span class="muted">추가 ${r.extraPeople}명</span>`:""}</td><td>${r.vehicleUse?`<b>${r.vehicleStatus||"사용"}</b><br><span class="muted">${vehicleTimeLabel(r.vehicleStart)} ~ ${vehicleTimeLabel(r.vehicleEnd)}</span>`:"미사용"}</td><td><span class="status ${reservationStatusLabel(r)}">${reservationStatusLabel(r)}</span><br><span class="muted">${r.source==="manual"?"관리자 등록":(r.status==="승인"?(r.checkinStatus||"이용대기"):(displayPaymentDue(r)||""))}</span></td><td class="actions">${admin?adminReservationButtons(r):userReservationButtons(r)}</td></tr>`).join("")}</tbody></table>`;
}
...
function stayUseApplicationTable(){syncStayApplications();const rows=state.stayUseApplications||[];if(!rows.length)return `<div class="panel empty">숙박 이용 신청서가 없습니다.</div>`;return `<div class="table-scroll"><table class="table"><thead><tr><th>구분</th><th>신청번호</th><th>신청자</th><th>숙박 시설</th><th>이용 기간</th><th>인원</th><th>생성일</th><th>상태</th><th>출력</th></tr></thead><tbody>${rows.map(a=>`<tr><td>숙박 이용 신청서</td><td>${a.applicationNo}</td><td>${a.userName}</td><td>${a.roomName}</td><td>${a.checkin} ~ ${a.checkout} (${a.nights}박)</td><td>${a.people}명</td><td>${a.createdAt}</td><td>${a.status}</td><td><button onclick="printStayUseApplication('${a.id}')">출력/PDF</button></td></tr>`).join("")}</tbody></table></div>`;}
