const APP_VERSION="V28.0.9 Production";
const RELEASE_DATE="2026-07-27";
const RELEASE_SUMMARY="V28.0.9 카카오 변수 · 예약 메모 · 반려사유 · 삭제 보안 개선";
...
function workflowVariables(row){
  ...
  return {"#{이름}":name,"#{회원명}":name,"#{신청자}":name,"#{신청자명}":name,"#{부서}":row.dept||"","#{숙소명}":row.roomName||room.name||"","#{숙박시설명}":row.roomName||room.name||"","#{체크인}":row.checkin||"","#{체크아웃}":row.checkout||"","#{박수}":String(nights),"#{예약기간}":period,"#{이용기간}":period,"#{숙박기간}":period,"#{예약번호}":reserveNo,"#{신청번호}":reserveNo,"#{이용인원}":String(row.people||0),"#{인원}":String(row.people||0),"#{복지항목}":row.type||"","#{신청항목}":row.type||"","#{신청금액}":amountText,"#{최종결제금액}":amountText,"#{입금금액}":amountText,"#{입금액}":amountText,"#{결제금액}":amountText,"#{은행명}":bankName,"#{계좌번호}":bankAccount,"#{예금주}":bankHolder,"#{입금계좌}":`${bankName} ${bankAccount} ${bankHolder}`.trim(),"#{입금기한}":due,"#{반려사유}":row.rejectReason||"","#{사유}":row.rejectReason||"","#{이벤트명}":row.type||row.title||"","#{행사일}":row.date||"","#{신청일}":date,"#{상태}":row.status||"","#{안내내용}":row.memo||row.condolenceContent||"복지몰에서 확인해 주세요."};
}
