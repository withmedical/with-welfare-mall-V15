const APP_VERSION="V28.0.8.2 Production";
const RELEASE_DATE="2026-07-27";
const RELEASE_SUMMARY="V28.0.8 시험 오류 보완 · 날짜 선택 · 입금안내 · 반려사유 · 신청서 목록 개선";
...
function reservationTable(rows,admin){
  ...
  return`<div class="table-scroll"><table class="table"><thead><tr>${admin?`<th>선택</th>`:""}<th>숙소</th><th>신청자</th><th>기간</th><th>구분/결제금액</th><th>인원</th>${admin?`<th>예약 메모</th>`:""}<th>차량</th><th>상태</th><th>관리</th></tr></thead><tbody>${rows.map(r=>`<tr>${admin?`<td>${rowCheck('chkReservation',r.id)}</td>`:""}<td>${r.roomName}</td><td>${r.userName}<br><span class="muted">${r.dept||""}</span></td><td>${r.checkin} ~ ${r.checkout}<br><b>${r.nights||calcNights(r.checkin,r.checkout)}박</b></td><td><b>${r.useType||"-"}</b><br>${Number(r.amount||0)>0?`<strong>${money(r.amount)}</strong>`:`<strong>무료</strong>`}</td><td>${r.people}명${r.extraPeople?`<br><span class="muted">추가 ${r.extraPeople}명</span>`:""}</td>${admin?`<td>${r.memo?String(r.memo).replace(/</g,"&lt;").replace(/>/g,"&gt;"):"-"}${r.rejectReason?`<br><span class="danger-text">반려사유: ${String(r.rejectReason).replace(/</g,"&lt;").replace(/>/g,"&gt;")}</span>`:""}</td>`:""}<td>${r.vehicleUse?`<b>${r.vehicleStatus||"사용"}</b><br><span class="muted">${vehicleTimeLabel(r.vehicleStart)} ~ ${vehicleTimeLabel(r.vehicleEnd)}</span>`:"미사용"}</td><td><span class="status ${reservationStatusLabel(r)}">${reservationStatusLabel(r)}</span><br><span class="muted">${r.source==="manual"?"관리자 등록":(r.status==="승인"?(r.checkinStatus||"이용대기"):(displayPaymentDue(r)||""))}</span></td><td class="actions">${admin?adminReservationButtons(r):userReservationButtons(r)}</td></tr>`).join("")}</tbody></table></div>`;
}
...
async function setStatus(type,id,status){
  ...
  let rejectReason="";if(status==="반려"){rejectReason=prompt("반려사유를 입력해 주세요.",row.rejectReason||"");if(rejectReason===null)return;if(!String(rejectReason).trim())return toast("반려사유를 입력해야 반려 처리할 수 있습니다.");rejectReason=String(rejectReason).trim();}
...
function workflowVariables(row){const name=row.userName||row.name||"";const date=row.date||row.eventDate||row.checkin||row.createdAt||"";const due=row.paymentDueAt||"";return {"#{이름}":name,"#{회원명}":name,"#{신청자}":name,"#{신청자명}":name,"#{부서}":row.dept||"","#{숙소명}":row.roomName||"","#{숙박시설명}":row.roomName||"","#{체크인}":row.checkin||"","#{체크아웃}":row.checkout||"","#{박수}":String(row.nights||calcNights(row.checkin,row.checkout)||0),"#{예약기간}":row.checkin&&row.checkout?`${row.checkin} ~ ${row.checkout}`:"","#{이용인원}":String(row.people||0),"#{복지항목}":row.type||"","#{신청항목}":row.type||"","#{신청금액}":money(row.amount||0),"#{최종결제금액}":money(row.amount||0),"#{입금금액}":money(row.amount||0),"#{입금액}":money(row.amount||0),"#{결제금액}":money(row.amount||0),"#{은행명}":row.bankName||state.settings.bankName||"","#{계좌번호}":row.bankAccount||state.settings.bankAccount||"","#{예금주}":row.bankHolder||state.settings.bankHolder||"","#{입금계좌}":row.bankInfo||`${row.bankName||state.settings.bankName||""} ${row.bankAccount||state.settings.bankAccount||""} ${row.bankHolder||state.settings.bankHolder||""}`.trim(),"#{입금기한}":due,"#{반려사유}":row.rejectReason||"","#{사유}":row.rejectReason||"","#{이벤트명}":row.type||row.title||"","#{행사일}":row.date||"","#{신청일}":date,"#{상태}":row.status||"","#{안내내용}":row.memo||row.condolenceContent||"복지몰에서 확인해 주세요."};}
