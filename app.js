const APP_VERSION="V28.0.6 Production";
...
function kakaoTemplateRows(){
 const n=state.notificationSettings||{},t=n.templateIds||{},enabled=n.templateEnabled||{},names=n.customTemplateNames||{};
...
function kakaoReadyPanel(){
 ...
 <div class="wide panel"><h4>담당자 알림 설정</h4><p class="muted">직원 이름을 쉼표로 구분해 입력합니다. 직원 회원정보의 휴대폰번호로 발송됩니다.</p><div class="grid3"><label>숙소 담당자<input name="managerStay" value="${kakaoHtml(((n.managerAssignments||{}).stay||['강익주']).join(', '))}"></label><label>복지 담당자<input name="managerWelfare" value="${kakaoHtml(((n.managerAssignments||{}).welfare||['이은아']).join(', '))}"></label><label>행사 담당자<input name="managerEvent" value="${kakaoHtml(((n.managerAssignments||{}).event||['이은아']).join(', '))}"></label></div></div>
...
async function saveKakaoSettings(e){e.preventDefault();const f=new FormData(e.target);const old=state.notificationSettings||{};const templateIds={...(old.templateIds||{})},customTemplateNames={...(old.customTemplateNames||{})};kakaoTemplateKeys().forEach(k=>{templateIds[k]=String(f.get('tpl_'+k)||'').trim();customTemplateNames[k]=String(f.get('tplname_'+k)||KAKAO_TEMPLATE_NAMES[k]||k).trim()||k;});state.notificationSettings={...old,kakaoProvider:f.get('kakaoProvider')||'SOLAPI',pfId:String(f.get('pfId')||'').trim(),senderPhone:onlyDigits(f.get('senderPhone')),testPhone:onlyDigits(f.get('testPhone')),solapiApiKey:String(f.get('solapiApiKey')||'').trim(),solapiApiSecret:String(f.get('solapiApiSecret')||''),templateIds,customTemplateNames,templateEnabled:{...(old.templateEnabled||{})},testTemplateKey:String(f.get('testTemplateKey')||'SN_WELCOME'),managerAssignments:{stay:String(f.get('managerStay')||'').split(',').map(x=>x.trim()).filter(Boolean),welfare:String(f.get('managerWelfare')||'').split(',').map(x=>x.trim()).filter(Boolean),event:String(f.get('managerEvent')||'').split(',').map(x=>x.trim()).filter(Boolean)},kakaoEnabled:f.get('kakaoEnabled')==='true'};safeLocalSave();const ok=await forceCloudSaveNow('카카오 설정');toast(ok?'카카오 설정과 템플릿 정보를 서버에 저장했습니다.':'카카오 설정은 현재 브라우저에 저장했습니다.');render();}
...
function workflowUserPhone(row){if(!row)return"";if(row.phone)return onlyDigits(row.phone);if(row.contact)return onlyDigits(row.contact);const u=(state.users||[]).find(x=>x.id===row.userId);return onlyDigits(u&&u.phone);}
...
async function sendWorkflowKakao(kind,action,row){try{const vars=workflowVariables(row),jobs=[],phone=workflowUserPhone(row),userKey=workflowTemplateKey(kind,action,row,false);if(phone&&userKey)jobs.push(sendKakaoTemplate(phone,userKey,vars,{kind:`${kind} ${action} 신청자`,targetId:row.id}));if(action==="received"){for(const m of managerRecipients(kind)){const key=workflowTemplateKey(kind,action,row,true);if(key)jobs.push(sendKakaoTemplate(m.phone,key,{...vars,"#{담당자명}":m.name},{kind:`${kind} 접수 담당자`,targetId:row.id}));}}if(jobs.length)await Promise.all(jobs);}catch(err){console.warn("카카오 자동발송 실패",err);}}
