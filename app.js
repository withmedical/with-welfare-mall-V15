const APP_VERSION="V28.0.7 Production";
...
async function confirmPayment(id){
  ...
  try{setOperationBusy(true,"입금확인 및 예약 확정 중입니다.");await patchServerRecord("reservations",id,patch);Object.assign(r,patch);addReservationTimeline(r.id,"입금 확인 및 예약 확정",`${r.userName||""} / ${r.roomName||""} / ${money(r.amount||0)}`);safeLocalSave();await sendWorkflowKakao("stay","approved",r);setOperationBusy(false);toast("입금 확인 및 예약 확정 처리되었습니다.");render();}catch(err){setOperationBusy(false);toast("처리 실패: "+(err.message||"오류"));}
}
...
function notificationAdminGroup(){const logs=state.kakaoLogs||[];return `<div class="group-box"><div class="group-section"><div class="subtle-title"><h3>카카오 알림 설정</h3><span class="muted">V28.0.7 Production</span></div>${kakaoReadyPanel()}</div><div class="group-section"><div class="subtle-title"><h3>발송 로그</h3><span class="muted">성공·실패·재발송</span></div>${logs.length?`<div class="actions" style="margin-bottom:12px"><button class="secondary" onclick="toggleAllKakaoLogChecks(this)">전체 선택</button><button class="danger" onclick="deleteSelectedKakaoLogs()">선택삭제</button><button class="danger" onclick="clearKakaoLogs()">전체삭제</button></div><div class="table-scroll"><table class="table"><thead><tr><th><input type="checkbox" id="kakaoLogAll" onchange="setAllKakaoLogChecks(this.checked)" aria-label="전체 선택"></th><th>일시</th><th>대상</th><th>종류</th><th>템플릿</th><th>상태</th><th>관리</th></tr></thead><tbody>${logs.map(x=>`<tr><td><input type="checkbox" class="kakao-log-check" value="${kakaoHtml(x.id)}" aria-label="로그 선택"></td><td>${kakaoHtml(x.createdAt||'')}</td><td>${kakaoHtml(x.to||'')}</td><td>${kakaoHtml(x.kind||'')}</td><td><code>${kakaoHtml(x.templateId||'')}</code></td><td>${kakaoHtml(x.status||'')}<br><span class="muted">${kakaoHtml(x.error||'')}</span></td><td><button class="secondary" onclick="resendKakao('${kakaoHtml(x.id)}')">재발송</button></td></tr>`).join('')}</tbody></table></div>`:`<div class="panel empty">발송 로그가 없습니다.</div>`}</div></div>`;}
...
function workflowVariables(row){const name=row.userName||row.name||"";const date=row.date||row.eventDate||row.checkin||row.createdAt||"";return {"#{이름}":name,"#{회원명}":name,"#{신청자}":name,"#{신청자명}":name,"#{부서}":row.dept||"","#{숙소명}":row.roomName||"","#{체크인}":row.checkin||"","#{체크아웃}":row.checkout||"","#{예약기간}":row.checkin&&row.checkout?`${row.checkin} ~ ${row.checkout}`:"","#{복지항목}":row.type||"","#{신청항목}":row.type||"","#{신청금액}":money(row.amount||0),"#{이벤트명}":row.type||row.title||"","#{행사일}":row.date||"","#{신청일}":date,"#{상태}":row.status||"","#{안내내용}":row.memo||row.condolenceContent||"복지몰에서 확인해 주세요."};}
function workflowTemplateKey(kind,action,row,manager=false){if(kind==="stay"){if(row&&row.userType==="customer"&&!manager)return action==="received"?"SN_STAY_RECEIVED":(action==="approved"?"SN_STAY_APPROVED":"SN_STAY_REJECTED");return action==="received"?"WM_STAY_RECEIVED":(action==="approved"?"WM_STAY_APPROVED":"WM_STAY_REJECTED");}if(kind==="welfare")return action==="received"?"WM_WELFARE_RECEIVED":(action==="approved"?"WM_WELFARE_APPROVED":"WM_WELFARE_REJECTED");if(kind==="event")return action==="received"?"WM_EVENT_RECEIVED":(action==="approved"?"WM_EVENT_APPROVED":"WM_EVENT_REJECTED");return"";}
async function sendWorkflowKakao(kind,action,row){try{const vars=workflowVariables(row),jobs=[],phone=workflowUserPhone(row),userKey=workflowTemplateKey(kind,action,row,false);if(userKey)jobs.push(sendKakaoTemplate(phone,userKey,vars,{kind:`${kind} ${action} 신청자`,targetId:row.id}));if(action==="received"){const managerKey=workflowTemplateKey(kind,action,row,true);for(const m of managerRecipientsForTemplate(managerKey,kind)){jobs.push(sendKakaoTemplate(m.phone,managerKey,{...vars,"#{담당자명}":m.name},{kind:`${kind} 접수 담당자`,targetId:row.id}));}}await Promise.all(jobs);}catch(err){console.warn("카카오 자동발송 실패",err);}}
...
function managerRecipientsForTemplate(key,kind){const n=state.notificationSettings||{};const raw=(n.templateManagers||{})[key];const names=String(raw||"").split(",").map(x=>x.trim()).filter(Boolean);if(!names.length)return managerRecipients(kind);return (state.users||[]).filter(u=>names.includes(u.name)&&u.status==="가입승인"&&isValidMobile(u.phone));}
